  ...
  implements SDKInitHook, BeforeRequestHook, AfterErrorHook
{
  ...
  private async getCredentialsGlobal(
    ...
    return {
      clientID: out?.OAuth2?.clientID ?? env().QTI_CLIENT_ID ?? "",
      clientSecret: out?.OAuth2?.clientSecret ?? env().QTI_CLIENT_SECRET ?? "",
      tokenURL: out?.OAuth2?.tokenURL ?? env().QTI_TOKEN_URL ?? "",
    };
  }
