...
import { QtiDefaultError } from "../models/errors/qtidefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | QtiDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | QtiDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new QtiDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new QtiDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
